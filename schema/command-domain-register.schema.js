...
const inputSchema = schemaValidator
  ...
  .keys({
    ...
    // reservation [optional] [0|1 defaults to 0].
    // If “1” the domain will be reserved, if “0” or not present,
    // the domain will be registered.
    reservation: schemaValidator.number().valid([0, 1]),

    // c_registrant [string] [mandatory]: The registrant contact ID
    // (a registrant must be created before registering the domain).
