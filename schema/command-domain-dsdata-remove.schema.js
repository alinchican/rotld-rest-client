...
const inputSchema = schemaValidator
  ...
  .keys({
    ...
    // keytag [numeric] [optional] has values between 0 & 65535
    keytag: schemaValidator
      .number()
      .min(0)
      .max(65535),

    // alg [numeric] [optional]:
    ...
    // digest [string] [optional]:  has only hexadecimal characters.
    ...
    digest: schemaValidator
      ...
      .when("digest_type", {
        is: "1",
        then: schemaValidator.string().length(40)
      })
      .when("digest_type", {
        is: "2",
        then: schemaValidator.string().length(64)
      })
  });
